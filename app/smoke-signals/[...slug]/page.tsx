...
export default function SmokeSignalsGamePage() {
  ...
  if (gameState.phase === 'lobby') {
    ...
    return (
      <Centered>
        ...
        {showRef && <CardReference version="all" onClose={() => setShowRef(false)} />}
        {showRules && (
          <RulesReference
  ...
  return (
    <div className="ss-elevated min-h-screen px-4 py-6">
      ...
      {showRef && <CardReference version={g.version} onClose={() => setShowRef(false)} />}
      {showRules && (
        <RulesReference
          version={g.version}
...
// `'all'` shows every character, sectioned by player count (lobby).
function CardReference({
  ...
  return (
    ...
    >
      <div
        className="mx-auto max-w-5xl rounded-2xl border border-stone-700 bg-stone-900 p-5 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-extrabold text-white">Card reference</h2>
          <button
            onClick={onClose}
            className="rounded-lg bg-stone-800 px-3 py-1.5 text-sm font-semibold text-stone-200 hover:bg-stone-700"
          >
            ✕ Close
          </button>
        </div>
...
// How-to-play summary, grounded in the engine (see lib/games/smoke-signals/logic.ts).
// Shown from both the lobby and the in-game header.
function RulesReference({
  ...
  return (
    ...
    >
      <div
        className="mx-auto max-w-2xl rounded-2xl border border-stone-700 bg-stone-900 p-5 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-stone-400">
              {version === 'regular' ? 'Regular' : 'Premium'} deck · send your message through the smoke and outlast the circle
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-5 text-sm leading-relaxed text-stone-300">
          ...
          <RuleSection title="🔥 On your turn">
          ...
          <RuleSection title="💨 Getting knocked out">
          ...
          <RuleSection title="🌙 How a round ends">
            <ul className="list-disc space-y-1 pl-5">
          ...
          <RuleSection title="♥ Earning tokens">
            <ul className="list-disc space-y-1 pl-5">
...
function RuleSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h3 className="mb-1.5 font-[family-name:var(--font-cinzel)] text-base font-semibold tracking-wide text-red-300">
        {title}
      </h3>
      {children}
    </section>
